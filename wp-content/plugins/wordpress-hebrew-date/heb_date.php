...
function hebDate($sDate) {
  ...
  if (($sJewMonth == "אדר" || $sJewMonth == "אדרא'" || $sJewMonth == "'אדרא") && hasLeapYear($juldate)) $sJewMonth='אדר א׳';
  if ($sJewMonth == "אדרב" || $sJewMonth == "אדרב'" || $sJewMonth == "'אדרב") $sJewMonth = "אדר ב׳";
...
function return_today_hebDate(){
  ...
  return hebdate($today);
}
add_shortcode('today_hebdate', 'return_today_hebDate');
//include("admin.php");
...
<input type="text" name="hebdate_format_custom" value="<?php echo(hebdate_format());?>" onfocus="hebdate_format_custom_radio.checked=true" size="10" dir="ltr"/> <?php echo(format(hebdate_format(),hebdate($example),mysql2date(get_option('date_format'), $example)));?><br/>
	...
	<p><?php printf(__('You can to find the longitude and the latitude via %s, via %s or via %s.','hebdate'),
		'<a href="https://maps.google.com/">'.__('Google maps','hebdate').'</a>',
			...
			'<a href="https://www.latlong.net/">LatLong.net</a>')?></p>
	</td>
	<tr valign="top">
