...
Class JNews_View_Counter
{
    ...
    public function query($instance)
    {
        ...
        if(is_array($instance['post_type'])) {
            $post_type_str = '(';                                       // Nadav Edition
            foreach ($instance['post_type'] as $index => $post_type) {
                $post_type_str .= $index === 0 ? "'$post_type'" : ",'$post_type'";
            }
            $post_type_str .= ')';
            $where .= " AND p.post_type IN {$post_type_str}";
        } else {
            $where .= " AND p.post_type = '{$instance['post_type']}'"; // original
        }
