...
Class ModuleQuery {
	...
	private static function default_query( $attr ) {
        ...
        // Added by Nadav
        if (isset($attr['meta_query'])) {
            $args['meta_query'] = $attr['meta_query'];
        }
        // Added by Nadav
        if (isset($attr['tax_query'])) {
        ...
        if ($args['post_type'] === 'post') {
            $args['post_type'] = array('post', 'opinion', 'page');
        }
//Nina remove - makes all queries Archive - does not appear in old jnews
	...
	private static function custom_query( $attr ) {
		if ( function_exists( 'jnews_view_counter_query' ) ) {
            ...
            if ($args['post_type'] === 'post') {
                $args['post_type'] = array('post', 'opinion', 'page');
//                $args['post_type'] = 'post\' OR p.post_type = \'opinion';
            }
