...
Class Carousel_2_View extends CarouselViewAbstract {
	public function content( $results ) {
		...
		foreach ( $results as $key => $post ) {
            ...
            if ($value==1)
             $icon="<img class =\"movie_icon_carousel\" src=\"".get_parent_theme_file_uri('/assets/img/movie_icon_carousel.png'). "\"/>";
            else if ($audio==1)
                $icon="<img class=\"movie_icon_carousel\" src=\"".get_parent_theme_file_uri('/assets/img/audio_icon_carousel.png'). "\"/>";
            else 
              $icon="";
            if ($stampa==1)
