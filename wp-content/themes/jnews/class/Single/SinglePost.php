...
//        $this->authors = get_field('writers', $this->post_id);
        $orderWriter = get_post_meta($this->post_id, 'orderWriter', true);

        if($orderWriter !== NULL){
            $this->authors = wp_get_object_terms($this->post_id, 'writer', array('orderby'=>'term_order'));
        }else{
            $this->authors = wp_get_post_terms($this->post_id, 'writer');
        }

        foreach ($this->authors as $author) {
            $author->image = get_field('image', 'writer_' . $author->term_id);
        }
		$this->hook();
	}
    public function getAutorsData(){
        return $this->authors;
    }

    /**
     * Get field from authors, names with more than 1 will be separate by commas
     * @param $fieldName
     * @return string
     */
    public function getAuthors($fieldName) {
        if(empty($this->authors)) {
            return '';
        }

        // for multiple names
        if($fieldName === 'name' && count($this->authors) > 1) {
            $str = '';
        ...
        return $this->authors[0]->{$fieldName};
    }
    ...
    public function getAuthorsCount() {
        return empty($this->authors) ? 0 : count($this->authors);
    }
	...
	public function main_content_width() {
		...
		if ( in_array( $layout, array( 'right-sidebar', 'left-sidebar' ) ) ) {
            ...
            if ( ! is_active_sidebar( $sidebar ) ) {
               ...
               // file_put_contents('./log_layout.log', $layout.' '.$width.PHP_EOL, FILE_APPEND);
                return $width;
            }
        }
       ...
       // file_put_contents('./log_layout.log', $width .''.$layout.PHP_EOL, FILE_APPEND);
		return $width;
	}
	...
	public function render_inline_related_post( $content ) {
		if ( get_post_type() === 'post' && is_single() && ! is_admin() ) { //Nina Remove - not in old jnews
