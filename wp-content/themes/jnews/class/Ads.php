...
Class Ads
{
    ...
    private function __construct()
    {
        ...
        add_action('jnews_before_main', array($this, 'after_header')); //Nina Add front page ad
    }
    ...
    {
        if((get_post_type() === 'post' || get_post_type() === 'opinion') && is_single() && ! is_admin())
        {
...
//    }
    public function prefix_insert_after_paragraph( $insertion, $paragraph_id, $content )
    {
        ...
        // find all block level elements in content
        preg_match_all("/\<\/(blockquote|br|form|div|figure|embed|form|h1|h2|h3|h4|h5|h6|p|ul|video|object|dl|ol|map)+\>/", do_shortcode($content) , $matches);
        $elements = $matches[1];

        $paragraphs = explode($begin_p, $content);
        if(count($paragraphs) > $paragraph_id ){
            $req_paragraph_id = $paragraph_id;
            ...
            foreach ($paragraphs as $index => $paragraph) {
                //fixed by oshri
                // check whether the next element is also p tag and if the new paragraph id is not more then 3 paragraph ahead
                //find next occurrence of p tag in elements array
                if (($paragraph_id - 1) == $index) {
                    for($i = $paragraph_id - 1;$i<count($elements);$i++){
                        if($elements[$i]== "p"){
                            break;
                        }
                    }
                    ...
                    } else if($req_paragraph_id + 2 >= $paragraph_id){
                        $paragraph_id++;
                    }
                }
                if (trim($paragraph)) {
    ...
    public function after_header($echo = true)
    {
        echo jnews_sanitize_output($this->render_ads('after_header','jeg_ad jeg_after_header container'));
    }
    ...
    //add custom script to display DFP ads in mobile with conditions
    public function sticky_footer_ads()
    {
        $sticky_ad="";
        if(is_front_page()) {
            $sticky_ad = "<div id='dfp_mobile_hp_sticky' class=\"makor_mobile_ad makor_ad\">
                     <script type='text/javascript'>
                      googletag.cmd.push(function() { googletag.display('dfp_mobile_hp_sticky'); });
                      </script>
                       </div>";
        }
        else if(is_singular( array('post','opinion'))) {
            $sticky_ad = "<div id='dfp_mobile_ip_sticky' class=\"makor_mobile_ad makor_ad\">
                    <script type='text/javascript'>
        ...
        $html = "<div class=\"jeg_ad jnews_mobile_sticky_ads " . $this->additional_class('mobile_sticky') . "\"><div class=\"ads-wrapper\"><div class=\"ads_code\">" .  $sticky_ad . "</div></div></div>";
        echo jnews_sanitize_output( $html );
    }
...
//    public function inline_module_2()
//    {
//        echo jnews_sanitize_output( $this->render_ads('inline_module_2') );
//    }
    ...
    public function render_ads($location, $addclass = '', $default = array())
    {
        ...
        //$enabled = true; //Nina - need to remove
        $ads_html = '';
